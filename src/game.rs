...
impl Game {
    ...
    pub fn unmove(&mut self) {
        let move_ = self.history.pop().unwrap_or_else(|| panic!("No more moves to pop"));
        self.board[move_.row][move_.column] = -1;
        if move_.row == 0 {
            self.nonFull.push(move_.column);
        }
    }
    #[wasm_bindgen]
